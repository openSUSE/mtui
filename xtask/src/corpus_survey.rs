//! `cargo xtask corpus-survey` — reads the in-flight update queue
//! (`GET /api/v1/updates?status=testing`), then issues one `GET
//! /api/v2/reports/{id}` per id, and reports the status
//! split (200 / 404 / 409 stale / 503 `generating` / other) broken down by
//! RRID kind. This sizes T5 (the legacy-corpus backfill) in the umbrella
//! plan, plus the stale-document backlog that must be regenerated by hand.
//! Read-only: no `Authorization` header, no write verb.
...
/// (2026-09-12) found it is not noise — every
...
fn classify(is_ok: bool, status: Option<u16>) -> ReportStatus {
    ...
    match status {
        ...
        Some(409) => ReportStatus::Stale409,
...
impl KindCounts {
    fn record(&mut self, status: ReportStatus) {
        match status {
            ...
            ReportStatus::Stale409 => self.stale_409 += 1,
...
mod tests {
    ...
    fn classify_maps_known_statuses_and_falls_back_to_other() {
        ...
        assert_eq!(classify(false, Some(409)), ReportStatus::Stale409);
