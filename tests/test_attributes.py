...
class Test_from_search_hosts_query(object):
  def make_attrs(ctx, q):
    return Attributes.from_search_hosts_query(q)
  ...
  def testcase_5(ctx):
    ...
    eq_(a.archs, list())
    eq_(a.addons, dict())
    eq_(a.virtual, dict(hypervisor = 'xen', mode = ''))
    eq_(a.kernel, None)
    eq_(a.ltss, None)
    eq_(a.minimal, None)
  ...
  def testcase_6(ctx):
    ...
    eq_(a.archs, list())
    eq_(a.addons, dict())
    eq_(a.virtual, dict(hypervisor = 'xen', mode = 'host'))
    eq_(a.kernel, None)
    eq_(a.ltss, None)
    eq_(a.minimal, None)
  ...
  def testcase_7(ctx):
    ...
    eq_(a.archs, list())
    eq_(a.addons, dict())
    eq_(a.virtual, dict(hypervisor = 'xen', mode = 'guest'))
    eq_(a.kernel, None)
    eq_(a.ltss, None)
    eq_(a.minimal, None)

  def testcase_8(ctx):
    q = 'kernel !ltss webyast'
    a = ctx.make_attrs(q)
    eq_(a.product, '')
    eq_(a.major, None)
    eq_(a.minor, None)
    eq_(a.archs, list())
    eq_(a.addons, dict(webyast = dict()))
    eq_(a.virtual, dict(hypervisor = '', mode = ''))
    eq_(a.kernel, True)
    eq_(a.ltss, False)
    eq_(a.minimal, None)
