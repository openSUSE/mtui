...
from mtui.data_sources.slack import ReviewOutcome, SlackClient
...
class TestSlackClientCalls:
    ...
    @responses.activate
    def test_chat_post_message_returns_ts_and_posts_channel_text(
        self, client, mock_config
    ):
        """chat_postMessage returns the ts and posts the right channel/text."""
        responses.add(
            responses.POST,
        ...
        ts = client.chat_postMessage("C123", "please review")
    ...
    def test_chat_post_message_threads_reply(self, client, mock_config):
        ...
        responses.add(
            responses.POST,
            _api(mock_config, "chat.postMessage"),
            json={"ok": True, "ts": "1700000000.000200"},
            status=200,
        )
...
class TestWaitForAck:
    ...
    def test_streams_replies_then_acks_with_reviewer(self, client, mock_config):
        ...
        responses.add(
            ...
            json={
                "ok": True,
                "messages": [{"text": "the request"}, {"text": "looking now"}],
            },
            status=200,
        )
        responses.add(
