...
| `pi_autolock` | bool | `true` | When testing a Product Increment (PI), lock each refhost as it connects while the report is loaded, released on unload/quit. |
...
| `audit_log` | path | *(unset)* | Durable audit sink for tool calls (versioned JSONL, one record per call, `0600`). Unset disables auditing. A path that cannot be opened refuses to start the server; a call the sink cannot record later is refused. See [MCP server](mcp.md#audit-log). |
