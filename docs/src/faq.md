...
`update` refuse with an error rather than reporting `prepare completed on
<hosts>` over a no-op that installs nothing (#396). Check
...
`SRCRPMs`, `Bugs`, `Creator`. Fields the TeReGen queue listing does not carry
(`Package-Streams`, `Issues`, `Comments`) are named in the error rather than
silently absent. `Unassigned Roles` renders `n/a` when it cannot
