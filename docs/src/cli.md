...
Every command below is available in the `mtui` REPL and non-interactive single-command mode. Commands not on the [MCP deny-list](mcp.md) are also exposed as `mtui-mcp` tools. Each command additionally accepts the shared template flags `-T/--template <RRID>` and `--all-templates`.
