...
* **Package data paths** (:func:`terms_path`) — the on-disk location of the
  ``terms/`` directory shipped inside the installed ``mtui`` package.
  Resolved via :mod:`importlib.resources`.
...
def _package_data_path() -> Path:
    ...
    This is the root from which the ``terms/`` subdirectory can be reached.
    """
    pkg = files("mtui")
