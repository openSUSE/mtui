...
class TestopiaEdit(Command):
    ...
    def run(self):
        ...
        keywords = ['summary', 'automated', 'status',
                    'requirement', 'setup', 'breakdown', 'action', 'effect']
        candidates = []
        ...
        if not candidates:
            ...
            self.log.warning(
                'Possible candidates found: {!s}. Please be more specific'.format(candidates))
            return
        ...
        try:
            ...
            self.log.error(
                'failed to modify testcase {!s}'.format(candidates[0]))
        else:
            self.log.info(
                'testcase saved: {!s}/tr_show_case.cgi?case_id={!s}'.format(self.config.bugzilla_url, candidates[0]))
