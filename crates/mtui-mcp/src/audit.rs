...
/// Whether two `(dev, ino)` pairs name the same file.
///
/// An inode number is unique only within its device, so the device has to be
/// part of the identity or a bind mount can make two files look like one.
#[cfg(unix)]
fn same_identity((dev_a, ino_a): (u64, u64), (dev_b, ino_b): (u64, u64)) -> bool {
...
fn same_file(a: &std::fs::File, b: &std::fs::File) -> io::Result<bool> {
    ...
    Ok(same_identity((a.dev(), a.ino()), (b.dev(), b.ino())))
}
...
/// Quoting a shell line or a JSON payload wraps a key in, trimmed from both
/// ends before the key is looked up so `-H "Authorization: …"`,
/// `'{"password":"…"}'` and the structured `env[PASSWORD]=…` are still
/// recognised. A credential name never contains any of them.
const KEY_WRAPPERS: [char; 8] = ['\'', '"', '{', '}', '[', ']', '(', ')'];
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn same_identity_needs_both_device_and_inode() {
        for (a, b, expected) in [
            ((1, 10), (1, 10), true),
            ((1, 10), (1, 11), false),
            // Inode numbers repeat across devices, so a bind mount can hand
            // the reopened sink the same one on a different filesystem.
            ((1, 10), (2, 10), false),
        ] {
            assert_eq!(same_identity(a, b), expected, "{a:?} vs {b:?}");
        }
    }
    ...
    fn mask_secrets_table() {
        for (raw, expected) in [
            ...
            // A structured key puts the credential name inside brackets.
            ("env[PASSWORD]=secret", "env[PASSWORD]=<redacted>"),
            ("params[token]=secret", "params[token]=<redacted>"),
            ("data[api_key]=x", "data[api_key]=<redacted>"),
