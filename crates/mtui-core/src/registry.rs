...
///
/// Names not yet backed by a registered command (`edit`, `terms`) are reserved
/// for their later waves; [`mcp_denylist_is_consistent`] tolerates them so adding
/// the command later does not require touching this list.
pub const MCP_DENYLIST: &[&str] = &[
    ...
    "edit", "terms", // later waves; reserved
];
...
mod tests {
    ...
    fn register_all_command_count() {
        ...
        // follow-ups (reload_openqa, set_workflow: mtui-rs-zs4/plt) + 1 Phase 6
        // (help: mtui-rs-lhz.9) = 59 canonical commands.
        assert_eq!(register_all().len(), 59);
    }
    ...
    fn mcp_denylist_is_consistent() {
        ...
        // REPL-only set (quit+aliases, switch, shell) plus `help` (Phase 6).
        // Reserved names (edit/terms) are not yet registered.
        ...
        assert_eq!(
            registered_denied,
            vec!["quit", "exit", "EOF", "switch", "shell", "help"]
        );
    }
