...
/// Builds the [`CheckoutError::SvnCheckoutFailed`] for `rrid`, deriving the
/// log URL from `config.fancy_reports_url`.
...
pub async fn svn_commit_testreport(
    ...
    // The `results` add is unchecked; a missing/awkward `results` dir must
